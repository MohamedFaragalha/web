...
if(isset($_POST['add_to_cart'])){
   ...
   $select_cart = mysqli_query($con, "SELECT * FROM `add cart` WHERE name = '$product_name' AND user_id = '$user_id'") or die('query failed');

   if(mysqli_num_rows($select_cart) > 0){
      $message[] = 'المنتج أضيف بالفعل إلى عربة التسوق!';
   }else{
      mysqli_query($con, "INSERT INTO `add cart`(user_id, name, price, image, quantity) VALUES('$user_id', '$product_name', '$product_price', '$product_image', '$product_quantity')") or die('query failed');
...
if(isset($_POST['update_cart'])){
   ...
   mysqli_query($con, "UPDATE `add cart` SET quantity = '$update_quantity' WHERE id = '$update_id'") or die('query failed');
   $message[] = 'تم تحديث كمية سلة التسوق بنجاح!';
}

if(isset($_GET['remove'])){
   $remove_id = $_GET['remove'];
   mysqli_query($con, "DELETE FROM `add cart` WHERE id = '$remove_id'") or die('query failed');
   header('location:index.php');
}
  
if(isset($_GET['delete_all'])){
   mysqli_query($con, "DELETE FROM `add cart` WHERE user_id = '$user_id'") or die('query failed');
   header('location:index.php');
}
...
<div class="shopping-cart">
   ...
   <table>
      ...
      <tbody>
      <?php
         $cart_query = mysqli_query($con, "SELECT * FROM `add cart` WHERE user_id = '$user_id'") or die('query failed');
         $grand_total = 0;
